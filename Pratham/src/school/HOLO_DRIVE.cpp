...
void HOLO::To() // Possibly can add another parameter that asks for turning degree more implementation details below
{
  ...
  while (1) 
  {
    ...
    LF.spin(fwd, voltsLF, voltageUnits::volt); // After turning calculations, the motor turning volts calculated can be added/subtracted in the parameter of these spin functions.
    LB.spin(fwd, voltsLR, voltageUnits::volt);
    RF.spin(fwd, voltsRF, voltageUnits::volt);
    RB.spin(fwd, voltsRR, voltageUnits::volt);
